        ...
        <v-layout justify-center align-center fill-height column>
            <v-flex xs12>
                <v-card width="700" flat class="card pa-3 mb-2 primary--text">
                    <v-layout row>
                        <v-flex xs9>
                            <p class="medium mt-4 ml-2 text-xs-left">Created By Anonymous</p>
                        </v-flex>
...
import { mapGetters } from 'vuex'
  export default {
    ...
    async mounted() {
        try {
            const response = await this.$axios.get(`api/get-result/ ${this.getAuthUserId}/peer- ${this.$route.params.id}`)
